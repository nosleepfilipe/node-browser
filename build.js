  ...
  driverConfig.forEach(function (item) {
    ...
    download
      ...
      .run(function (err, files) {
        if (err) {
          throw new Error(
            "Download drivers error, please reinstall " + err.message
          );
        }
...
function reworkDest(downloadFilePath, compressDir, type) {
  ...
  var binName =
    type + (process.platform === "win32" ? ".exe" : "-" + process.platform);
  var binSrcPath = path.resolve(
    compressDir,
    type === "phantomjs" ? "./bin/phantomjs" : "./chromedriver"
  );
